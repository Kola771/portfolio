    ...
    <main>
        <div class="my-[1.5rem] flex-wrap max-w-[95%] lg:my-[3rem] md:my-[2rem] md:max-w-[90%] lg:max-w-[80rem] lg:p-6 m-auto flex lg:gap-8"
            ...
            <section class="lg:basis-[45%] text-[#333] font-serif">
                <h2 class="font-bold mt-2 text-[2rem] lg:text-[4rem] md:text-[3rem]">Bibliographie</h2>
                <h3 class="lg:text-[2rem] text-[20px] md:text-[22px] mt-3 mb-4 uppercase">Koladé ABOUDOU M.</h3>
        ...
        <div class="my-[1.5rem] max-w-[95%] lg:my-[3rem] lg:max-w-[80rem] md:my-[2rem] md:max-w-[90%] lg:p-6 m-auto flex lg:gap-8"
            ...
            <section>
                ...
                <div class="flex flex-wrap md:gap-2 lg:gap-4 gap-3 justify-center">
                    <a :href="`${projet.linkSite}`" target="blank"
                        class="w-full lg:w-[400px] md:w-[320px] h-full relative lienSite" v-for="projet in allProject"
                        :key="projet.id">
                        <figure class="h-[500px]">
                            <img class="h-full" :src="`/storage/projetImage/${projet.image}`" alt="my_project">
                            <figcaption class="caption">
                                <div class="max-w-[100%] my-10 mx-auto p-[2rem] flex flex-col">
                                    <h4 class="my-3 lg:my-3 md:my-2 lg:text-[22px] text-center">{{ projet.name_project }}
                                    </h4>
                                    <p class="my-3 lg:my-4 lg:text-justify text-center">{{ projet.description }}</p>
                                    <p class="my-2 lg:my-4 lg:text-justify text-center">Poste: {{ projet.poste }}</p>
                                    <div class="my-4 lg:text-justify text-center">Langages utilisés :
                                        <p class="my-3 lg:mt-4 md:my-3 lg:text-justify text-center">Pour le frontend: <span
                                                class="font-bold uppercase">{{ projet.frontend }}</span>
                                        </p>
                                        <p class="my-3 lg:mt-4 md:my-3 lg:text-justify text-center">Pour le backend: <span
                                                class="font-bold uppercase">{{ projet.backend }}</span></p>
                                    </div>
                                </div>
                            </figcaption>
                        </figure>
...
export default {
    beforeCreate() {
        // Fonction exécutée avant le chargement de la page
        axios.get(route("allProjectLimit")).then(response => {
            this.allProject = response.data;
            if (this.allProject.length == 3) {
                this.condition = true;
            }
        })
    },
